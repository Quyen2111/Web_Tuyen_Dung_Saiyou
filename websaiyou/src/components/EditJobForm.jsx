...
const EditJobForm = ({ job, onClose, onUpdate }) => {
  const initialState = {
    company: "",
    title: "",
    salary: "",
  ...
  useEffect(() => {
    if (job) {
      ...
      const formattedDate = job.date ? new Date(job.date).toISOString().split("T")[0] : ""

      // Initialize form data with job data or defaults
      setFormData({
        company: job.company || "",
        title: job.title || "",
        salary: job.salary || "",
        ...
        date: formattedDate,
        time: job.time || "",
        rank: job.rank || "Nhân viên",
        quantity: job.quantity || "",
        form: job.form || "Toàn thời gian",
        degree: job.degree || "Không yêu cầu",
        experience: job.experience || "Không yêu cầu",
        professional: job.professional || "",
        description: Array.isArray(job.description) && job.description.length > 0 ? job.description : [""],
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://6811d48d3ac96f7119a5c04f.mockapi.io/TuyenDung/jobDatas/${job.id}`, {
        method: "PUT",
        headers: {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-6 border-b">
          <h3 className="text-xl font-semibold text-gray-900">Cập nhật tin tuyển dụng</h3>
